...
class cita_model{
    ...
    public function get_citas($fecha_){
        $fecha = mysqli_real_escape_string($this->db, $fecha_);
        $query ="select *, tramite from cita INNER join tramites ON tramites.id=cita.id_tramites where cita.fecha='{$fecha}' and cita.estado=1;";
    ...
    public function update_cita($id_cita){
      $id= mysqli_real_escape_string($this->db, $id_cita);
      $query = "update cita set estado=0 where id_cita={$id};";
      $consulta=$this->db->query($query);
